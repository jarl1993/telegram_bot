...
def obtener_numero():
    url = "https://www.uma.es/pas/cms/menu/empleo/bolsas-de-trabajo/bolsas/"
    try:
    ...
    number_element = soup.find('td', valign="middle")
    if number_element:
        strong_element = number_element.find('strong')
        if strong_element:
            return strong_element.text.strip()
    return None
...
# Función para monitorizar el número
def monitorizar_numero():
    global ultimo_numero
    nuevo_numero = obtener_numero()
    if nuevo_numero and nuevo_numero != ultimo_numero:
        logger.info(f"Nuevo número detectado: {nuevo_numero}")
        ultimo_numero = nuevo_numero

# Configurar el programador
scheduler = BackgroundScheduler(timezone=pytz.timezone('Europe/Madrid'))
scheduler.add_job(monitorizar_numero, 'interval', hours=6)
scheduler.start()
